...
void updateWatchedLiterals(int assertedVar) {
    ...
    for (auto clauseIndex = copy.begin(); clauseIndex != copy.end(); ++clauseIndex) {
        ...
        if (eval(clause[0]) || eval(clause[1])) continue;
    ...
    if (numOfUnassigned == 0 && unitQueue.empty()) pthread_exit(0);
}

bool eval(int literal) { return !(vars[std::abs(literal)].getValue() ^ (literal > 0)); }